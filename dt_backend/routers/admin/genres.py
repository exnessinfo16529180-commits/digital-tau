...
def create_admin_genres_router(engine: Engine) -> APIRouter:
    ...
    @router.get("/admin/genres", response_class=HTMLResponse)
    def admin_genres(request: Request):
            ...
            items.append(
                ...
                <tr>
                  ...
                  <td style="padding:10px;border-bottom:1px solid #222;">
                    <form style="display:inline" method="post" action="/admin/genres/{r["id"]}/delete"
        ...
        body = f"""
          <div style="display:flex;gap:12px;align-items:end;margin-bottom:14px;">
            <form method="post" action="/admin/genres/new" style="display:flex;gap:10px;align-items:end;flex-wrap:wrap;">
              <div>
    ...
    @router.post("/admin/genres/new")
    def admin_genres_create(request: Request, name: str = Form(...)):
        require_login(request)

        clean = (name or "").strip()
        if not clean:
            return RedirectResponse("/admin/genres", status_code=302)
